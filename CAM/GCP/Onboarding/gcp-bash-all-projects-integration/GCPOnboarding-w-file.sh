...
if [[ -z "$PROJECTS_FILE" ]]; then
  echo "Error: Debes especificar el archivo que contiene los proyectos usando la variable PROJECTS_FILE."
  exit 1
fi
...
process_project() {
  ...
  if [[ -z "$project_id" ]]; then
    echo "Error: No se encontró el ID del proyecto para el nombre: $project_name"
    return
  fi
...
tail -n +1 "$PROJECTS_FILE" | while IFS= read -r project_name; do
  process_project "$project_name" "$v1_account_id" "$api_key" "$VISION_ONE_ENDPOINT"
done
